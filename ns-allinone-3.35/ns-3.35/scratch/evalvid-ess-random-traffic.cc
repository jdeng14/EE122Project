...
int main (int argc, char *argv[]) {
  ...
  const std::string bandwidth("75Kbps");
  ...
  //Uses a hardcoded receiver for now. 
  port = 2000;
  UdpServerHelper serverUdp (port);
  apps = serverUdp.Install (wifiUserNodes[2].Get (2));//p2pNodes.Get(1));
  apps.Start (Seconds (2.0));
  apps.Stop (Seconds (300.0));

  uint32_t MaxPacketSize = 1024;
  UdpTraceClientHelper clientUdp(userInterfaces[2].GetAddress(2), port, "");//p2pInterfaces.GetAddress(1), port, "");
  clientUdp.SetAttribute ("MaxPacketSize", UintegerValue (MaxPacketSize));
  apps = clientUdp.Install (p2pNodes.Get(1));
  apps.Start(Seconds(5.0));
  apps.Stop(Seconds(300.0));
  /*
  //put on each device except evalvid device. 
  double start_diff = .1;
  double start_time = 5.1;
  for (int i=0;i<nCsma;i++) { 
    for (uint32_t j=0;j<wifiUserNodes[i].GetN();j++) { 
      start_time += start_diff;
      apps = clientUdp.Install (wifiUserNodes[i].Get (j));
